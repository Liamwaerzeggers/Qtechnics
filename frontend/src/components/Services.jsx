import React from 'react';
import { Home, Wrench, Layers } from 'lucide-react';
import { services } from '../data/mock';

const iconMap = {
  home: Home,
  wrench: Wrench,
  layers: Layers,
};

const Services = () => {
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service) => {
            const IconComponent = iconMap[service.icon];
            return (
              <div
                key={service.id}
                className="bg-gray-50 rounded-lg p-8 hover:shadow-lg transition-all duration-300 group cursor-pointer"
              >
                <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center mb-6 shadow-sm group-hover:shadow-md transition-shadow">
                  <IconComponent className="h-8 w-8 text-[#4a3728]" />
                </div>
                <h3 className="text-xl font-bold text-[#4a3728] mb-3">
                  {service.title}
                </h3>
                <p className="text-[#6a5748]">{service.description}</p>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
};

export default Services;
