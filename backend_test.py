...
class OfferteAPITester:
    def __init__(self, base_url="https://realtor-estimates.preview.emergentagent.com/api"):
...
%%EOF"""
    ...
    def test_customer_portal(self):
        ...
        try:
            ...
            visible_docs = [doc for doc in legacy_docs_list if doc.get('visible_to_customer') == True]
            hidden_docs = [doc for doc in legacy_docs_list if doc.get('visible_to_customer') != True]
            
            if len(hidden_docs) > 0:
                print(f"❌ Found {len(hidden_docs)} documents with visible_to_customer=false in customer portal")
                return False
            
            print(f"✅ All {len(visible_docs)} documents are correctly visible to customer")
